...
SELECT
    ...
    COALESCE(dcat.Category_SK,-1)        AS Category_SK,

    -- Supplier (SCD4: resolved through Product, current row)
    COALESCE(dsup.Supplier_SK,-1)        AS Supplier_SK,

    -- Shipper (SCD1: ShipVia is the natural key)
    COALESCE(dsh.Shipper_SK,  -1)        AS Shipper_SK,

    -- Territory (SCD3: single row per NK)
    COALESCE(dt.Territory_SK, -1)        AS Territory_SK,

    -- Region (SCD1: resolved through Territory)
    COALESCE(dr.Region_SK,    -1)        AS Region_SK,
