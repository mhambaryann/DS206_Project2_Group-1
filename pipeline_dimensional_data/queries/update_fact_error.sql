...
-- ============================================================
-- STEP 2: Insert rows where at least one required dimension
--         lookup returned NULL. The lookup set mirrors
--         update_fact.sql, so every row excluded from FactOrders
--         because of missing dimension keys is auditable here.
-- ============================================================
...
SELECT
    ...
    -- Category and supplier are resolved through the current product row;
    -- region is resolved through the territory row.
    CONCAT(
        ...
        CASE WHEN dp.Product_SK   IS NULL
             ...
             ELSE '' END,
        CASE WHEN dp.Product_SK IS NOT NULL AND dcat.Category_SK IS NULL
             THEN 'Missing CategoryID='  + ISNULL(CAST(dp.CategoryID_NK AS NVARCHAR), 'NULL') + '; '
             ELSE '' END,
        CASE WHEN dp.Product_SK IS NOT NULL AND dsup.Supplier_SK IS NULL
             THEN 'Missing SupplierID='  + ISNULL(CAST(dp.SupplierID_NK AS NVARCHAR), 'NULL') + '; '
             ELSE '' END,
        ...
        CASE WHEN dt.Territory_SK IS NULL
             ...
             ELSE '' END,
        CASE WHEN dt.Territory_SK IS NOT NULL AND dr.Region_SK IS NULL
             THEN 'Missing RegionID='    + ISNULL(CAST(dt.RegionID_NK AS NVARCHAR), 'NULL') + '; '
             ELSE '' END
    )                                    AS ErrorReason,
...
-- Only rows where at least one required dimension lookup failed
  AND (
        dc.Customer_SK   IS NULL
     OR de.Employee_SK   IS NULL
     OR dp.Product_SK    IS NULL
     OR dcat.Category_SK IS NULL
     OR dsup.Supplier_SK IS NULL
